...
import com.nkdata.gwt.streamer.client.impl.*;
...
public abstract class Streamer
{
	...
	private final static Map<String,String> classIdMap = new HashMap<String, String>();
	/** reverse class id -> class name mapping */ 
	private final static Map<String,String> idClassMap = new HashMap<String, String>();
	/** package name hints */
	private final static List<String> packages = new ArrayList<String>();
	 ...
	 * @return generatedId
	 */
	public synchronized static String createAlias( Class<?> cl ) {
		String id = generateNewClassId();
		classIdMap.put( cl.getName(), id );
		idClassMap.put( id, cl.getName() );
		aliasHash ^= hashCode(id);
		recalculateStreamVersion();
		return id;
	}
	...
	public synchronized static void registerStreamer( Class<?> targetClass, Streamer streamer )
	{
		...
		registeredStreamers.add(targetClass.getName());
		recalculateStreamVersion();
	}
	 ...
	 */
	public synchronized static void addPackageHint( String packageName )
	{
	...
	*/
	...
	public String toString( final Object obj )
	{
		...
		return streamVersionMark + out.toString();
	}
	...
	public Object fromString( final String str ) 
	{
		...
		final Reader in = streamFactory.createReader( str.substring(streamVersionMark.length()) );
		final String strVerMark = str.substring(0,streamVersionMark.length());
		if ( !streamVersionMark.equals( strVerMark ) )
			throw new StreamerException( "The message was serialized with another version of stream. " +
					...
					"Please, ensure that both have followed in the same order createAlias(), registerStreamer(), addPackageHint(), " +
